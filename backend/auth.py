...
def login(username: str, password: str):
    users = list_documents(USERS_COLLECTION_ID, filters=[f"username={username}"])["documents"]
...
def register(username: str, password: str, wallet_address: str):
    users = list_documents(USERS_COLLECTION_ID, filters=[f"username={username}"])["documents"]
    if users:
        raise HTTPException(status_code=400, detail="Username exists")
    user = create_document(
        USERS_COLLECTION_ID,
        {"username": username, "password": password, "wallet_address": wallet_address}
    )
    token = jwt.encode({"sub": user["$id"]}, SECRET_KEY, algorithm="HS256")
    return {"access_token": token, "token_type": "bearer"}